...
function ProfileDataTable({currentPersons, persons, loading = false, searchQuery = "", offset = 0, personsPerPage = 10, status="",pagination,
              ...
              <div className="card-title-group d-flex justify-content-between align-items-center">
                <div className="card-title">
                  <h5 className="title">
                    ...
                    <span className="badge badge-info ml-2">{persons.filter(person=>person.status===status).length}</span>
                  </h5>
                </div>
              ...
              <table className="table table-hover nowrap align-middle dataTable-init">
                ...
                <tbody style={{ fontFamily: "Segoe UI" }} className="tb-tnx-body">
                  {loading ? (
                    ...
                    currentPersons.filter(person=>person.status===status).map((person, index) => (
                      <tr key={person._id}>
                        ...
                        <td
                          onClick={() => gotoUserDetail(person.pid ?? '')}
                        ...
                        <td>{person.cnic}</td>
                        <td><span className="badge badge-info">{person.gender==""?"N/A":person.gender.toUpperCase()}</span></td>
                        <td><span className="badge badge-success">{person.birth_place.toUpperCase()}</span></td>
