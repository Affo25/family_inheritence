              ...
              <table className="table table-hover nowrap align-middle dataTable-init">
                ...
                <tbody style={{ fontFamily: "Segoe UI" }} className="tb-tnx-body">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan="11" className="text-center">No marriages record found</td>
                    </tr>
                  ) : (
                    persons.map((person, index) => (
                      <tr key={person._id}>
                        <td><b>{offset + index + 1}</b></td>
                         <td>{person.rid}</td>
                        <td>{person.person1_id}</td>
                        <td>{person.person2_id}</td>
                         <td><span className="badge badge-info">{person.relationship_type.toUpperCase()}</span></td>
                        <td><span className="badge badge-info">{person.status.toUpperCase()}</span></td>
                        <td><span className="badge badge-success">{new Date(person.created_on).toLocaleDateString()}</span></td>
                        <td><span className="badge badge-info">{new Date(person.updated_on).toLocaleDateString()}</span></td>
...
export default MarriageDataTable